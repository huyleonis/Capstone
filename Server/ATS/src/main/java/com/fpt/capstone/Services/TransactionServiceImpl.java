...
public class TransactionServiceImpl implements TransactionService {
    ...
    public TransactionDTO insertManualTransaction(String licensePlate, int laneId) {
        ...
        int transaction = transactionRepos.insertManualTransaction(id, stationId, now, id, price.getPrice(),
                TransactionType.MANUAL.getType(), vehicle.getId());
        if (transaction > 0) {
    ...
    public TransactionDTO insertAutoTransaction(String username, int stationId) {
        ...
        int transaction = transactionRepos.insertAutoTransaction(id, stationId, now,
                TransactionStatus.TRANS_PENDING.getName(), price.getPrice(), TransactionType.AUTOMATION.getType(),
                vehicleId);
    ...
    @Override
    public TransactionDTO createCapturedTransaction(String plate, String photo, Date createdTime, int stationId)
            throws Exception {
        ...
        if (vehicle != null) { // Nếu số xe có trong db
            ...
            int result = transactionRepos.createCaptureTransaction(id, stationId, createdTime,
                    TransactionStatus.TRANS_INITIAL.getName(), price.getPrice(), photo, vehicle.getId());
